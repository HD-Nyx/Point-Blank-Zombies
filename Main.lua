--Rename this script to startup.lua [rename Main.lua startup.lua]

local function StartGame()

    local StarterGun = "pointblank:glock17"
    local StarterAmmo = "pointblank:ammo9mm"
    local StarterAmmoAmount = 34
    ...
    local function SpawnNewWave()
        if CanDoWave == true then
            sleep(0.1)
            WaveInProgress = true
            commands.exec('/execute at @e[type=minecraft:armor_stand,tag=PlayerSpawner,limit=1] run tp @a[gamemode=spectator] ~ ~ ~')
            commands.exec('/tp @a[gamemode=spectator] @e[type=minecraft:armor_stand,tag=PlayerSpawner,limit=1]')
            ...
            for i = 1, AmountOfZombies do
                if WaveInProgress == true then
                    SpawnRandomZombie(HealthMultiplier, RunnerChance, BruteChance)
                    --At wave 1, delay is 2.95, at wave 50, delay is 0.5 (cap)
    ...
    local function AllPlayersDead()
        while true do
            ...
            if not PlayersThere then
                ...
                commands.exec('/playsound minecraft:pointblankzombies.gameover master @a ~ ~ ~ 1.5 1')
                commands.exec('/stopsound @a master minecraft:pointblankzombies.ambiance1')
                sleep(9)
                ...
                commands.exec('/gamemode @a adventure')
                commands.exec('/clear @a')
                sleep(0.1)
                os.shutdown()
            end
    ...
    parallel.waitForAll(AllZombiesDead, AllPlayersDead)

end

while true do
    sleep(0.1)
    if commands.exec('/execute if score #zombies StartGame matches 1') == true then
         commands.exec('/scoreboard players set #zombies StartGame 0')
        StartGame()
    end
end